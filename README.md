...
В альтернативном режиме просмотра область видимости хранится в KV хранилище, отсортированном по возрастанию по ключу.
Ключем в данном случае выступает модуль разности между рейтингом игрока и рейтингом планеты. 
Значением является другая коллекция, хранящая позиции планет в  том порядке, в котором они добавлялись по мере действий игрока.
...
Однако в зависимости от значения зума на UI возвращается то хранилище, которое соответствует текущему режиму просмотра. 
 ...
 ##### Ручное управление памятью SpaceTile'ов
  ...
  В текущей реализации этим занимается GC, хотя эту ответственность можно было перенести на SpaceGridTileCache.
  
    NativeArray будет хорошим вариантом (однако требуется доработать SpaceTileIO).
 ...
 ##### Улучшение способа хранения планет, видимых в альтернативном режиме
  На текущий момент альтернативный режим хранит все планеты, содержащиеся в области видимости, но показывает только определенное кол-во из них. 
  Данный момент можно оптимизировать. 
...
Для корретной работы алгоритма достаточно хранить:
1) Все граничные элементы, помеченные X (границы квадрата видимой области).
2) Наиболее близкие с т.з. рейтинга планеты в кол-ве P среди всех элементов в области, помеченной * (внутренний квадрат). 

При передвижении и зуме необходимо следить за содержимым внутреннего квадрата и поддерживать его актуальным.  
...
Выше приведен пример зума, который расширяет как границы видимой области, так и внутренний квадрат. 
После этого действия свойство внутреннего квадрата (хранение только P значений) должно соблюдаться.

  ##### Запись и чтение с диска тайлов внутри области видимости
  
  Помимо тайлов вокруг области видимости в некоторых случаях нам так же нет нужды держать в памяти тайлы внутри области видимости (см. Улучшение способа хранения планет, видимых в альтернативном режиме). 
